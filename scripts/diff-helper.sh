...
case "$CMD" in
  ...
  branch)
    ...
    ;;
  base-candidates)
    # List candidate base branches (main, master, release_prod, prod, prd, develop)
    git branch -r --list '*/main' '*/master' '*/release_prod' '*/prod' '*/prd' '*/develop' 2>/dev/null | sed 's|.*/||' | sort -u
    ;;
  *)
